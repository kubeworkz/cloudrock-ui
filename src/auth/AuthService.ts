 ...
 * user session would be cleaned both in Cloudrock and authentication backend.
 *
 ...
 *    he is redirected back to the Cloudrock UI.
 *
...
import { cleanObject } from '@cloudrock/core/utils';
...
function logout() {
  ...
  if (
    ...
    ENV.plugins.CLOUDROCK_AUTH_SAML2.ENABLE_SINGLE_LOGOUT
  ) {
    store.dispatch(
      showSuccess(
        translate(
