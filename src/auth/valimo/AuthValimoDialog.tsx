...
import { showErrorResponse, showError } from '@cloudrock/store/notify';
    ...
    const authenticateValimo = async (formData) => {
      try {
        const { message, uuid } = await login(
          ENV.plugins.CLOUDROCK_AUTH_VALIMO.MOBILE_PREFIX.concat(
      ...
      <form onSubmit={handleSubmit(authenticateValimo)}>
        ...
        <ModalBody>
          <FormGroup>
            ...
            <InputGroup>
              <InputGroup.Addon>
                {ENV.plugins.CLOUDROCK_AUTH_VALIMO.MOBILE_PREFIX}
              </InputGroup.Addon>
              <Field
                type="tel"
                name="phoneNumber"
